			...
			<?php echo "<h2>" . __( 'Facetly Configuration' ) . "</h2>"; ?>  
		  	...
		  	<form name="facetly_fields" method="post" action="<?php echo str_replace( '%7E', '~', $_SERVER['REQUEST_URI']); ?>">  
				...
				?> 
			        ...
			        <tr>
			            <td>
			            	<select name="<?php echo $def_value; ?>" style="width: 191px;">
				            <?php
								$selected = "";
			    ...
			    <?php
			    	...
			    	$facetly_fields = get_option('facetly_fields');
		    		$checked['post'] = "";
	    			$checked['wpsc-product'] = "";
			    	if (!empty($facetly_fields['node_type'])) {
			    		if (in_array('post', $facetly_fields['node_type'])) {
			    			$checked['post'] = "checked";
			    		} 
			    		if (in_array('wpsc-product', $facetly_fields['node_type'])) {
			    			$checked['wpsc-product'] = "checked";
			    		} 
			    	}
			    ...
			    ?>
			    	...
			    	<tr>
			    		<td>
			    			<input type="checkbox" name="node_type[]" value="post" <?php echo $checked['post']; ?>/> post<br />
							<input type="checkbox" name="node_type[]" value="wpsc-product" <?php echo $checked['wpsc-product']; ?>/> wp ecommerce 
			    		</td>
			    	</tr>
			      	<tr>
				        <td>
