...
function facetly_fields()
{
    ...
    if (empty($fields)) {
        ...
        foreach ($custom_fields as $custom_value) {
            if (@unserialize($custom_value->meta_value) === true)
                continue;
        ...
        foreach ($fields_data->field as $fields_value) {
            if ($fields_value->name == 'url' || $fields_value->name == 'imageurl' || $fields_value->name == 'category')
                continue;
...
?> 
            ...
            <div class="wrap">  
                <?php
        echo "<h2>" . __('Facetly Configuration') . "</h2>";
?>  
                <?php
        echo "<h4>" . __('Facetly Fields') . "</h4>";
?>  
                <form name="facetly_fields" method="post" action="<?php
